...
parser = OptionParser()
parser.add_option("--accession", dest="accession", action="store", type="string",
                  help="Accession is the RefSeq accession for the desired reference genome used in the contig reordering step (mandatory for draft genomes)",
                  metavar="ACCESSION")
parser.add_option("--token", dest="token", action="store", type="string",
                  help="Your authentication token for the API. This is found under'JOBS' > 'HTTP API Token' when you login at www.pathogenomics.sfu.ca/islandviewer and is valid for 30 days before having to be refreshed.",
                  metavar="TOKEN")
parser.add_option("--gbk", dest="gbk", action="store", type="string",
                  help="Provide a name for the reordered genbank file containing genomic islands predicted by IslandViewer (output file).",
                  metavar="GENBANK")
parser.add_option("--seq", "--sequence", dest="sequence", action="store", type="string",
                  help="Provide a name for the sequence file (genbank format) used as input.", metavar="SEQUENCE")
parser.add_option("--tab", dest="tab", action="store", type="string",
                  help="Provide a name for the tab-delimited file that summarizes predicted genomic islands (output file).",
                  metavar="TAB")
...
multipart_data = MultipartEncoder(fields={"format_type": "GENBANK", 'ref_accnum': options.accession,
...
try:
    ...
    response = requests.post(host + "/rest/submit/", headers=headers, data=multipart_data, timeout=120)
    if not response.ok:
    ...
    os.system(
        "curl --silent -H 'x-authtoken: " + options.token + "' -Fformat_type=GENBANK -F ref_accnum=" + options.accession + "  -Fgenome_file=@" + options.sequence + " -X POST " + host + "/rest/submit/ > response.json")
